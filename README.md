...
To maximize performance and keep resource utilization low, the native launcher automatically optimizes headless Chrome instances with flags like `--mute-audio` and `--disable-gpu` (when WebGPU is not requested). This typically reduces memory usage by up to 120 MB per active agent session. For complete deployment guidelines, refer to the [Resource Sizing Guide](SIZE.md).
